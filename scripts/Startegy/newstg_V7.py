...
class OptionVolatility(object):
        ...
        for i in range(len(self.result)-3): # 改成while,不断获取数据
            ...
            if position==0 and cash!=0:
                if self.StrongUpSignal_V3(vol_list):
                    ...
                    #挂单未成交，position不改变
                    alltradeqty,cash,pnl=self.bidActionCheck(i,cash)
                    if alltradeqty!=0:
                        position=1
                ...
                position=0
                alltradeqty,cash,pnl=self.CloseActionCheck(i,position,alltradeqty)
    ...
    def bidActionCheck(self,num,cash):
        ...
        if self.result['high'].iloc[num+3]>=self.ask_price and self.result['low'].iloc[num+3]<=self.ask_price: # 挂单成交
            ...
            # 吃单优化：待处理（以下部分程序无用）
            if self.signal_record[-1] in ['SC']:
            #    print('挂单未成交，以实时最高价吃单!')
                self.ask_price = self.result['low'].iloc[num+3]
                alltradeqty = cash* (1 - 0.04 / 100)/(self.ask_price) # Taker按0.04%收手续费
                cash = 0
                return [alltradeqty,cash,pnl]
            elif self.signal_record[-1] in ['LC']:
            #    print('挂单未成交，以实时最低价吃单!')
                self.ask_price = self.result['high'].iloc[num+3]
                alltradeqty = cash* (1 - 0.04 / 100)/(self.ask_price) # Taker按0.04%收手续费
                cash = 0
                return [alltradeqty,cash,pnl]
            else:
    ...
    def CloseActionCheck(self,num,position,alltradeqty):
        ...
        if self.result['high'].iloc[num+3]>=self.ask_price and self.result['low'].iloc[num+3]<=self.ask_price: # 挂单成交
            ...
            pnl = alltradeqty * (self.ask_price - self.trading_result['askprice'].iloc[num-1])*(self.trading_result['position'].iloc[num-1]-position) - self.ask_price*alltradeqty*0.02/100
            cash = pnl +  self.trading_result['alltradeqty'].iloc[num-1] * self.trading_result['tradeprice'].iloc[num-1]
        else:
    ...
    def StrongUpSignal_V3(self,vol_list):
        ...
        if len(self.signal_record)!=0:
            if self.signal_record[-1]=='SC' and returns.iloc[0]>=0:
                self.ask_price=vol_list['low'].iloc[1]
                # vol_list=[i,i+1,i+2] 其实时间已经走到了i+2, 这个时候拿到的bid价格，要看是否成交，需要判断i+3时刻的情况，但是i+3，作为15min线，又可以拆分成三块，在这三块都有high/low的情况下，一旦属于该区间范围，则成交
                # i+2时刻的close价格其实等于i+3时刻的open价格，如果high>=open,open总会在[low,high]区间内，=
                #print('======出现强上涨信号,挂单,价格:$'+str(self.ask_price)+',准备策略做多======')
                return True
            elif returns.iloc[0]>=0 and returns.iloc[1]>returns.iloc[0]: # 连续上涨，且上涨幅度越来越大
                self.ask_price=vol_list['close'].iloc[1] # 以长期横盘价挂单(暂以最后时刻的均价挂单) # 挂单成交验证，暂以close价挂
                #print('======出现强上涨信号,挂单,价格:$'+str(self.ask_price)+',准备策略做多======')
                return True
            else:
                pass 
        elif returns.iloc[0]>=0 and returns.iloc[1]>returns.iloc[0]: # 连续上涨，且上涨幅度越来越大
                self.ask_price=vol_list['close'].iloc[1] # 以长期横盘价挂单(暂以最后时刻的均价挂单)
                #print('======出现强上涨信号,挂单,价格:$'+str(self.ask_price)+',准备策略做多======')
                return True
        else:
            pass   
    ...
    def StrongUpSignal(self,vol_list):
        ...
        returns=vol_list['volatility']
        if len(self.signal_record)!=0:
            if self.signal_record[-1]=='SC' and returns.iloc[0]>=0 and returns.iloc[1]>returns.iloc[0]:
                self.ask_price=vol_list['close'].iloc[1] 
                # vol_list=[i,i+1,i+2] 其实时间已经走到了i+2, 这个时候拿到的bid价格，要看是否成交，需要判断i+3时刻的情况，但是i+3，作为15min线，又可以拆分成三块，在这三块都有high/low的情况下，一旦属于该区间范围，则成交
                # i+2时刻的close价格其实等于i+3时刻的open价格，如果high>=open,open总会在[low,high]区间内，=
                #print('======出现强上涨信号,挂单,价格:$'+str(self.ask_price)+',准备策略做多======')
                return True
        elif returns.iloc[0]>=0 and returns.iloc[2]>returns.iloc[1] and returns.iloc[1]>returns.iloc[0]: # 连续上涨，且上涨幅度越来越大
    ...
    def StrongUpSignal_V2(self,vol_list):
        # 强上涨信号
        returns=vol_list['volatility']
        if len(self.signal_record)!=0:
            if self.signal_record[-1]=='SC' and returns.iloc[0]>=0 and returns.iloc[1]>returns.iloc[0]:
                ...
                #print('======出现强上涨信号,挂单,价格:$'+str(self.ask_price)+',准备策略做多======')
                return True
            elif returns.iloc[0] >= 0 and returns.iloc[2] > returns.iloc[1] and returns.iloc[1] > returns.iloc[
                0]:  # 连续上涨，且上涨幅度越来越大
                self.ask_price = vol_list['close'].iloc[2]  # 以长期横盘价挂单(暂以最后时刻的均价挂单)
                # print('======出现强上涨信号,挂单,价格:$'+str(self.ask_price)+',准备策略做多======')
                return True
    ...
    def WeakUpSignal_V3(self,vol_list):
        ...
        returns=vol_list['volatility']
        if len(self.signal_record)!=0:
            if self.signal_record[-1]=='LC' and returns.iloc[0]>=0 and returns.iloc[1]>0: # 震荡行情，但不下跌
                self.ask_price=vol_list['close'].iloc[1] # 取三个时段的平均价挂单做多(暂以最后时刻的均价挂单)
                #print('======出现弱上涨信号,挂单,价格:$'+str(self.ask_price)+',准备策略做多======')
                return True
        return False

    def WeakUpSignal_V2(self,vol_list):
        # 弱上涨信号
        returns=vol_list['volatility']
        if len(self.signal_record)!=0:
            if self.signal_record[-1]=='SC' and returns.iloc[0]>=0 and returns.iloc[1]>0:
                self.ask_price=vol_list['close'].iloc[1] 
                #print('======出现弱上涨信号,挂单,价格:$'+str(self.ask_price)+',准备策略做多======')
                return True
            elif returns.iloc[0]>=0 and returns.iloc[1]>0 and returns.iloc[2]>0: # 震荡行情，但不下跌
    ...
    def WeakDownSignal(self,vol_list):
        ...
        returns=vol_list['volatility']
        if len(self.signal_record)!=0:
            if self.signal_record[-1]=='LC' and returns.iloc[0]<0 and returns.iloc[1]<0: # 时间已经走到下一刻i+2了，这里用历史数据判断的意义是？
                self.ask_price=vol_list['close'].iloc[1] 
    ...
    def ShortClosed(self,vol_list):
        ...
        if self.signal_record[-1] in ['SD','WD'] and returns.iloc[2]>0: #一旦下跌趋势中止就赶紧跑，不考虑什么波动率最大了
            # low不一定能吃到单，改成close吃单测试,file
            # 这里可以改成用5min的数据，去评估是否出现returns.iloc[2]>0，然后可以以low成交
            self.ask_price=vol_list['close'].iloc[1] # 之前是low,现核对挂单情况，全部修改为close
...
test_csv=["2020-all-BTC-5m.csv"]
#test_csv=["2019-all-BTC-5m.csv","2020-all-BTC-5m.csv","2021-all-BTC-5m.csv","2022-yearly-newest-BTC-5m.csv"]
for i in test_csv:
    secu_data_monthly = pd.read_csv(i) # 2022-yearly-newest-BTC-5m.csv or 2021-all-BTC-5m.csv
