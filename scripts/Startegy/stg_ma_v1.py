...
class OptionVolatility(object):
    def __init__(self, data, type_, freq, data_type):
        ...
        self.result = pd.DataFrame({'tradingtime': self.time, 'close': self.close, 'low': self.low, 'high': self.high,
                                    ...
                                    "same_percentage": same_percentage, 'ma7': self.ma7, 'ma25': self.ma25})  # 最终要输出的结果
    ...
    def process(self, cash):
        ...
        for i in range(len(self.result) - 3):  # 改成while,不断获取数据
            ...
            if position == 0 and cash != 0:
                if self.StrongUpSignal_V3(vol_list, i):
                ...
                elif self.StrongDownSignal(vol_list, i):
                    ...
                    pass
            elif self.LongClosed_V2(vol_list, i) and position == 1:
                self.signal_record.append("LC")
                position = 0
                alltradeqty, cash, pnl = self.CloseActionCheck_V2(i, position, alltradeqty)
                tradeprice = self.ask_price
            elif self.ShortClosed_V2(vol_list, i) and position == -1:
    ...
    def StrongDownSignal(self, vol_list, num):
        # 强下跌信号
        delta = 0
        returns = vol_list['volatility']
        if len(self.signal_record) != 0:
            if self.signal_record[-1] == 'LC' and self.ma7.iloc[num + 2] < self.ma25.iloc[num + 2]:
                self.ask_price = vol_list['close'].iloc[2]
                # TODO：理论上一定成交
                # print('======出现强下跌信号,挂单,价格:$'+str(self.ask_price)+',准备策略做空======')
                return True
        elif self.ma7.iloc[num + 2] < self.ma25.iloc[num + 2]:
            self.ask_price = vol_list['close'].iloc[2]
            # TODO：理论上一定成交
            # print('======出现强下跌信号,挂单,价格:$'+str(self.ask_price)+',准备策略做空======')
            return True
